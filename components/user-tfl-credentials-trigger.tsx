...
export const UserTflCredentialsTrigger = ({
  ...
  const { status, hydrated, appKeyMasked, openDialog } = useUserTflCredentials()
  ...
  return (
    <button
      ...
      onClick={openDialog}
      className={cn(
        "inline-flex h-8 w-full items-center gap-1.5 rounded-md px-2 text-xs text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring focus-visible:outline-none",
        className
      )}
      aria-label={
        ready
