...
export const SyntaxHighlightedCode = ({
  ...
  const codeInner = highlightedHtml ? (
    <code
      className={cn(
        "bg-transparent p-0 font-mono text-[0.925em] leading-normal text-foreground",
        className
      )}
  ...
  return (
    <div
      className={cn(
        "group/code relative mt-6 mb-4 w-full",
        isPeek && "mt-0 mb-0 overflow-hidden rounded-lg bg-muted",
        wrapperClassName
      )}
      {...(isPeek
        ? {
      ...
      {isPeek ? (
        <div className="relative z-10 -mt-3 flex justify-center pb-2">
          <button
            ...
            aria-expanded="false"
            className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border bg-background px-3 text-xs font-medium text-foreground shadow-xs hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
          >
...
export const MdxSyntaxPre = ({
  ...
  if (!code) {
    return (
      <pre
        className={cn(
          "mt-6 mb-4 block w-full overflow-x-auto overflow-y-hidden rounded-lg bg-muted p-4 text-sm",
          className
        )}
      >
        {children}
