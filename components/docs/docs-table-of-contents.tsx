...
const TOC_SKELETON_LINE_WIDTHS = [
  "w-28",
  "w-36",
  "w-24",
  "w-32",
...
const DocsTocSkeleton = () => (
  <>
    ...
    <ul className="space-y-1.5 border-l border-border" aria-hidden>
      {TOC_SKELETON_LINE_WIDTHS.map((width) => (
        <li key={width} className="py-0.5 pl-3">
 ...
 */
export const DocsTableOfContents = ({ className }: { className?: string }) => {
  ...
  if (!ready) {
    return (
      <nav aria-busy aria-label="On this page" className={railClassName}>
        <DocsTocSkeleton />
      </nav>
    )
  }
