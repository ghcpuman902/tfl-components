...
export const JUNCTION_TYPES: Record<JunctionTypeId, JunctionTypeInfo> = {
  linear: {
    ...
    name: "Linear",
    shape: "A─B─C",
    meaning: "Normal continuation; not a junction at all — the baseline every other type is judged against.",
    minLegs: 2,
  },
  "y-junction": {
    id: "y-junction",
    ...
    shape: "A─J─B, J─C",
    meaning: "A↔B and A↔C are valid; B↔C may not be. One leg reads as the operational branch off a through route.",
    minLegs: 3,
    establishedTerm: "trailing junction (or facing junction, by direction of travel)",
  },
  "through-fork": {
    id: "through-fork",
    name: "Through fork",
    shape: "A─J─B/C",
    meaning: "One incoming route divides into two onward routes that are both 'primary', not a trunk-plus-branch.",
    minLegs: 3,
  ...
  merge: {
    ...
    name: "Merge",
    shape: "A/B─J─C",
    meaning: "Two service paths converge onto one outgoing leg — a fork viewed in the converging direction.",
    minLegs: 3,
  ...
  crossing: {
    ...
    name: "Crossing",
    shape: "──┼──",
    meaning: "Physical lines cross in plan but share no track connection at all — no node, no movement.",
    minLegs: 4,
  ...
  "flat-junction": {
    ...
    shape: "──╋──",
    meaning: "Multiple approaches are physically connected, but only a defined subset of movements is permitted.",
    minLegs: 4,
  ...
  diamond: {
    ...
    shape: "4 approaches",
    meaning: "Through and turning movements are both possible; connectivity must be stated explicitly, not assumed from the shape.",
    minLegs: 4,
  ...
  wye: {
    ...
    shape: "△",
    meaning: "Three legs where every pairwise movement is possible, including reversal moves not used in passenger service.",
    minLegs: 3,
  ...
  loop: {
    ...
    meaning: "A route diverges and later reconnects to the same route further along — one physical circuit, not a dead end.",
    minLegs: 3,
  },
  spur: {
    id: "spur",
    name: "Spur",
    shape: "──┬─X",
    meaning: "A branch terminates rather than reconnecting; the far leg is a physical dead end, not just an unused movement.",
    minLegs: 3,
  },
  "multi-branch": {
    id: "multi-branch",
    ...
    meaning: "Generalisation of the wye/fork family to more legs than any single named shape covers.",
    minLegs: 4,
  ...
  "grade-separated": {
    ...
    meaning: "Routes cross geographically, but at least one movement is realised by a ramp/tunnel rather than a flat, conflicting merge.",
    minLegs: 3,
...
export const legIds = (junction: Junction): LegId[] => junction.legs.map((leg) => leg.id);
...
export const canMove = (junction: Junction, from: LegId, to: LegId): boolean =>
  junction.movements.some((movement) => movement.from === from && movement.to === to);
...
export const symmetricMovements = (pairs: readonly [LegId, LegId][]): Movement[] =>
  pairs.flatMap(([a, b]) => [
    { from: a, to: b },
    { from: b, to: a },
  ]);
...
export const validateJunction = (junction: Junction): string[] => {
  ...
  for (const movement of junction.movements) {
    if (!ids.has(movement.from)) {
      problems.push(`${junction.id}: movement references unknown leg "${movement.from}"`);
    }
    if (!ids.has(movement.to)) {
      problems.push(`${junction.id}: movement references unknown leg "${movement.to}"`);
    }
    if (movement.from === movement.to) {
      problems.push(`${junction.id}: movement from "${movement.from}" to itself`);
    }
  }
  if (junction.type === "crossing" && junction.movements.length > 0) {
    problems.push(`${junction.id}: crossing must have zero movements (no connection)`);
  }
  if (junction.rejoinsLegId && !ids.has(junction.rejoinsLegId)) {
    problems.push(`${junction.id}: rejoinsLegId references unknown leg "${junction.rejoinsLegId}"`);
  }
  for (const leg of junction.legs) {
    const usesLeg = junction.movements.some((m) => m.from === leg.id || m.to === leg.id);
    if (junction.type !== "crossing" && !usesLeg && junction.legs.length > 1) {
      problems.push(`${junction.id}: leg "${leg.id}" has no permitted movement at all`);
    }
  }
  return problems;
};
