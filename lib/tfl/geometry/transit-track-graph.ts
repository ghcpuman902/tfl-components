...
const groupByDirection = (polylines: readonly LngLat[][]): [LngLat[][], LngLat[][]] => {
  const ranked = [...polylines]
    .filter((coords) => coords.length >= 2)
    .sort((left, right) => lineLengthMetres(right) - lineLengthMetres(left));
...
const towardsForGroup = (
  ...
  return nearestStation(end, stations)?.label ?? nearestStation(end, stations)?.name;
};
...
const finishLineTracks = (
  ...
  for (const group of [0, 1] as const) {
    ...
    for (const shape of internedShapes(dualAccepted[group], intern, simplifyM)) {
      dualSimplified.push({
        coords: shape.coords,
        trackGroup: group,
        towards,
...
const groupFeaturesByLine = (
  ...
  const grouped = new Map<string, Feature<LineString, LineSegmentProperties>[]>();
  for (const feature of features) {
...
export const buildModeTracks = (
  ...
  for (const [lineId, lineFeatures] of grouped) {
    ...
    full.centreline.forEach((coords, index) => {
      centreFull.push(toLineFeature(coords, template, `${lineId}-track-${index}`));
    });
