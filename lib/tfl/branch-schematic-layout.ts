...
const compassPull = (from: StationLatLon, to: StationLatLon): CompassPull => {
  ...
  const above = NWSE_WEIGHT.n * Math.max(0, dLat) + NWSE_WEIGHT.w * Math.max(0, -dLon);
  const below = NWSE_WEIGHT.s * Math.max(0, -dLat) + NWSE_WEIGHT.e * Math.max(0, dLon);
  ...
  while (used.size < offshoots.length) {
    ...
    while (grew) {
      ...
      for (const index of remaining) {
        ...
        const canJoin =
          (side === "before" && (sides.has("after") || sides.has("parallel"))) ||
          (side === "after" && (sides.has("before") || sides.has("parallel"))) ||
          (side === "parallel" && (sides.has("before") || sides.has("after"))) ||
          (side === "before" && sides.has("before") && junction !== seedJunction) ||
...
const corridorLength = (
  ...
  trunkSet: ReadonlySet<string>,
): number => group.reduce((sum, offshoot) => sum + uniqueLength(offshoot, trunkSet), 0);
...
const segmentComponent = (
  ...
  while (stack.length > 0) {
    ...
    const neighbours = uniqueSorted([...current.nextIds, ...current.previousIds]);
...
const shouldDuplicate = (
  ...
  const incomingComponent = segmentComponent(stationId, incomingBranchId, segments);
...
const isInterchange = (stationId: string, lineId: string, degree: number): boolean => {
...
const layoutLoopSchematic = (
  ...
  const placed = new Map<string, { nodeId: string; lane: number; pos: number; branchIds: Set<string> }>();
  const place = (stationId: string, lane: number, pos: number, branchId: string) => {
  ...
  const loopBranch =
    topology.loopBranchIds?.[0] ?? topology.primaryBranchId;
  const spurBranch =
    topology.edges.find(
      (edge) =>
  ...
  for (const [index, stationId] of southArc.entries()) {
    ...
    const pos =
      stationId === opposite ? oppositePos : junctionPos + index;
  ...
  const branchMeta = new Map(topology.branches.map((branch) => [branch.id, branch]));
...
export const computeBranchSchematicLayout = (
  ...
  if (cycle && cycle.length / Math.max(1, topology.nodes.length) >= LOOP_COVERAGE) {
  ...
  const placeOffshoot = (offshoot: Offshoot, lane: number) => {
    ...
    for (let i = 0; i < ids.length; i += 1) {
      ...
      if (already && preferReuse && !shouldDuplicate(stationId, branchId, already, segments)) {
  ...
  for (const group of [...trunkAttached, ...laterLong]) {
    ...
    if (!existingAt(group[0]!.stationIds[0]!) &&
  ...
  for (const group of shortCorridors) {
    const attachment =
      ...
      existingAt(group[0]!.stationIds[group[0]!.stationIds.length - 1]!);
    const pull = orientation === "horizontal" ? pullOf(group) : { sign: 0 as const, strength: 0 };
  ...
  for (let i = 0; i < trunk.length - 1; i += 1) {
    ...
    const branchId =
      topology.edges.find(
        (edge) =>
          edge.fromStationId === trunk[i] &&
          edge.toStationId === trunk[i + 1],
      )?.branchId ?? topology.primaryBranchId;
  ...
  for (const placed of schematicNodesById.values()) {
    nodes.push({
      ...
      lane: placed.lane,
      pos: placed.pos,
      kind: nodeKind(placed.stationId, meta.lineId, degree.get(placed.nodeId) ?? 0),
