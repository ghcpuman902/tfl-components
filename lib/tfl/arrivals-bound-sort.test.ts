...
describe("parseCompassBoundId", () => {
  it("reads the compass prefix from platformName", () => {
    assert.equal(parseCompassBoundId("Westbound - Platform 1"), "westbound")
    assert.equal(parseCompassBoundId("Eastbound - Platform 2"), "eastbound")
...
describe("parseArrivalsPlatformLabel", () => {
  ...
  it("extracts a Platform N token from other descriptive labels", () => {
    ...
    assert.equal(parseArrivalsPlatformLabel("Chesham Branch - Platform 3"), "3")
  })
...
describe("formatBoundHeading", () => {
  ...
  it("spells out a platform-only heading", () => {
    ...
    assert.equal(formatBoundHeading({ platformLabel: "A" }), "Platform A")
  })

  it("keeps a direction-only heading when the platform is not hoisted", () => {
    assert.equal(formatBoundHeading({ boundId: "westbound" }), "Westbound")
  })
...
describe("compareArrivalsBounds", () => {
  ...
  it("orders West→East before North→South on a vertical board", () => {
    assert.ok(
      arrivalsBoundOrderKey("westbound") < arrivalsBoundOrderKey("eastbound")
    )
    assert.ok(
      arrivalsBoundOrderKey("eastbound") < arrivalsBoundOrderKey("northbound")
    )
    assert.ok(
      arrivalsBoundOrderKey("northbound") < arrivalsBoundOrderKey("southbound")
    )
  })
