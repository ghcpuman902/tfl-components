...
describe("colour tokens generator", () => {
  ...
  it("applies Go night OKLCH method in dark; Northern uses #FCFCFC", () => {
    ...
    assert.equal(cssVars.dark["tfl-line-northern"], northernDarkOklch())
    assert.equal(
    ...
    assert.equal(cssVars.theme["color-tfl-mono-ink"], "var(--tfl-mono-ink)")
  })
  ...
  it("keeps registry.json tfl-colours cssVars/css in sync", () => {
    ...
    const lineStrip = registry.items.find(
      (entry) => entry.name === "line-strip"
    )
    const lineStripFiles = (lineStrip as { files?: Array<{ target?: string }> })
      .files
    assert.ok(
      lineStripFiles?.some(
        (file) => file.target === "lib/tfl/bw-line-styles.ts"
      ),
      "line-strip should ship bw-line-styles.ts"
    )

    const lineBadge = registry.items.find(
