...
export async function buildLiveVehiclesSnapshot({
  ...
  const railIds = [...new Set(railLineIds.map((id) => id.trim()).filter(Boolean))];
  const busIds = [...new Set(busRouteIds.map((id) => id.trim()).filter(Boolean))];
  ...
  const fetchedAt = Math.max(railCached.fetchedAt, busCached.fetchedAt, Date.now());
    ...
    if (boundingBox) {
      try {
        ...
        bodsActivities = raw.filter((activity) => {
          ...
          return !Number.isFinite(recorded) || fetchedAt - recorded <= BODS_STALE_MS;
        });
      } catch {
        bodsActivities = [];
      }
