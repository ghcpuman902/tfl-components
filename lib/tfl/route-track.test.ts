...
describe("route-track", () => {
  it("maps cable car to triple rails and map red", () => {
    ...
    assert.equal(CABLE_CAR_DIAGRAM_COLOR, UNDERGROUND_LINE_COLOURS.central.hex)
  })
  ...
  it("resolves theme-aware CSS vars for known lines", () => {
    assert.equal(
      resolveDiagramLineCssColor("victoria"),
      "var(--tfl-line-victoria)"
    )
    assert.equal(
      resolveDiagramLineCssColor("northern"),
      "var(--tfl-line-northern)"
    )
    assert.equal(
      resolveDiagramLineCssColor("elizabeth"),
      "var(--tfl-mode-elizabeth)"
    )
    assert.equal(
