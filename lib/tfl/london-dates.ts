...
export const addLondonCalendarDays = (dateKey: string, days: number): string => {
  const start = londonDayStartMs(dateKey);
  const probe = new Date(start + 12 * 60 * 60 * 1000 + days * 24 * 60 * 60 * 1000);
 ...
 */
export const buildWeekAheadDays = (now: Date = new Date()): WeekAheadDayRange => {
