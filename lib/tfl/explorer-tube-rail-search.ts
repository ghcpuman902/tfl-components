...
export const TUBE_RAIL_LOCATE_RADIUS_METERS = 800
export const TUBE_RAIL_LOCATE_LIMIT = 25
...
const MODE_NEEDLES: ReadonlyMap<string, readonly string[]> = new Map(
  STATION_CATALOG_MODES.map((mode) => [
    mode.id,
    [fold(mode.id), fold(mode.label)].filter(Boolean),
  ])
)
...
/**
 * Filter the cached Tube & rail catalogue. Empty / punctuation-only queries
 * return the input order (A–Z). Hits are ranked name → id → line → mode,
 * then A–Z within a rank.
 */
export const filterExplorerTubeRailPoints = (
...
/**
 * Nearest catalogue stations to a geolocation. Points without coordinates
 * are skipped. Does not call TfL.
 */
export const nearbyExplorerTubeRailPoints = (
  points: readonly ExplorerPoint[],
  origin: { lat: number; lon: number },
  radiusMeters = TUBE_RAIL_LOCATE_RADIUS_METERS,
  limit = TUBE_RAIL_LOCATE_LIMIT
): ExplorerPoint[] => {
