...
const analyseRow = (row: GoColourRow): ContrastRow => {
  ...
  if (row.brandPlaceholder || row.goNight == null) {
    ...
    const apcaSeek =
      apcaGain != null && apcaGain >= 3 && Math.abs(brandOnNightApca) < 60
    // APCA soft-clips very dark-on-charcoal (Piccadilly); WCAG still moves.
    const wcagSeek =
      brandOnNightWcag < 3 && wcagGain != null && wcagGain >= 0.25
  ...
  return {
    ...
    goDayOnDayWcag: row.goDay ? wcagContrast(row.goDay, GO_DAY_PAPER) : null,
  }
}
...
const transformRow = (row: GoColourRow): TransformRow => {
  ...
  const brandToNight = nightO != null ? oklchDelta(brandO, nightO) : null
