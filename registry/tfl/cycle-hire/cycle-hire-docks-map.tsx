  ...
  <div className="relative" style={{ width: size, height: size }}>
    ...
    >
      <StationName
        ...
        allowScaleDown
        align="center"
        className="h-auto font-sans leading-none font-medium text-foreground"
      />
    </div>
  </div>
)
...
export const CycleHireDocksMap = ({
  ...
  const tileCount =
    tiles == null || !Number.isFinite(tiles)
      ? null
      : Math.max(1, Math.floor(tiles))
  const tileStyle =
    tileCount == null
      ? undefined
      : ({
          ...
          height: `calc(var(--arrivals-row) * ${tileCount})`,
        } as CSSProperties)

  useEffect(() => {
    const map = mapRef.current
    if (!map || tileCount == null) return
