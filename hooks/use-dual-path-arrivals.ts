...
import { withSharedTrackIdentity, type RealtimePrediction } from "tfl-ts"
import {
...
export const useDualPathArrivals = ({
  ...
  const familyKeys = (sharedTrackFamilies ?? [])
    .map((family) =>
      [...new Set(family.map((id) => id.trim()).filter(Boolean))]
        .sort()
        .join(",")
    )
  ...
  useEffect(() => {
    ...
    const startUserPoll = async () => {
      ...
      if (!appKey) {
        applyFailure("Add a TfL API key to load live arrivals with your quota.")
        return
      }
