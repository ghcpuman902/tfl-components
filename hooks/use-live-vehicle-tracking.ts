...
export type LiveVehiclesResponse =
  { ok: true; data: LiveVehiclesSnapshot } | { ok: false; error: string }
...
export const useLiveVehicleTracking = ({
  ...
  const railIds = useMemo(() => (railKey ? railKey.split(",") : []), [railKey])
  const busIds = useMemo(() => (busKey ? busKey.split(",") : []), [busKey])
  ...
  const userPollMs = computeBatchedPollIntervalMs({
    ...
    requestsPerTick:
      (railIds.length > 0 ? 1 : 0) + (busIds.length > 0 ? 1 : 0) || 1,
  })
  const pollIntervalMs = source === "user" ? userPollMs : SITE_POLL_MS

  const stationsById = useMemo(() => railStationsById(), [])
  const railPolylines = useMemo(() => railPolylinesForLines(railIds), [railIds])
  ...
  const placeSnapshot = (
    ...
    const gps =
      busPositionSource === "dead-reckoning"
        ? []
        : bodsActivitiesToVehicles(snapshot.bodsActivities ?? [], busIds, asOf)
