...
Pass `priorityLineIds` to keep specific lines open at the top of Service Disruptions. Other disrupted lines collapse to a title and severity chip — expand one to read it. Good Service always lists every line you fetched.

```tsx
<TubeStatusBoard data={data} now={fetchedAt} priorityLineIds={["central", "victoria"]} />
```
...
`compact` is the watchlist layout: one column, no **Service Disruptions** / **Good Service** title tiles, no attribution footer. Pair it with `hideHeader` when the parent already names the lines. `compact` shows only the lines you name — there is no "other lines" summary. To keep the full board but draw attention to a few lines instead, pass `priorityLineIds`.
...
`detailScope="network"` keeps every fetched line in Good Service and gives your `detailLineIds` lines a full reason; other disrupted lines get a brief identity frame instead of disappearing. `selection` shows only those lines. `none` drops the phase-wide chips.
...
| `priorityLineIds` | `readonly string[]` | Keep these lines expanded; others collapse. Omit to expand every line. |
