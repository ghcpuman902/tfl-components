...
Width steps need an `@container/arrivals` ancestor, as on [`RailArrivalsBoard`](/docs/tube-rail-arrivals). Visible form: `Platform N` → `Plat N` → `PN` → `N`. Casing stays title-style. `aria-label` stays `Platform N`.
...
| `compact` | `boolean` | Always paint `P{n}`. Use on a row when the subgroup heading is not the platform. |
