...
| `color` | `string` | Optional `--line-raw` override when `lineId` is outside the token set. |
| `className` | `string` | Merge classes. Diagram flags use `LINE_CHIP_DIAGRAM_CLASS`. |
| `style` | `CSSProperties` | Size overrides (`height`, `fontSize`, `minWidth`) for strip flag metrics. |
