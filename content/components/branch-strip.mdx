...
Lane joins follow the Line Diagram Standard, not the geographic Tube map: circular arcs at 45° / 90°, never freeform Béziers.

- **Horizontal** (in-carriage §6): corridor joins use a 45° S when space allows, otherwise 90° R. A true spur (degree-1 terminus into a corridor) is a single 45° diagonal with one R3x fillet. The terminus bar sits perpendicular to that diagonal.
- **Vertical** (platform §11): 90° branch curves only, R3n on the main and R4n on a spur.

Pass `mono` to paint B&W Tube-map stroke motifs on the graph. Mono scales through `x`, not `--tfl-diagram-scale`. Titles and chips stay in colour.
