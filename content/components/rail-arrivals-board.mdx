...
Pass `lineGroups` to merge lines that share platforms (for example Circle, Hammersmith & City, and Metropolitan at Liverpool Street). The section header uses the Line title **Shared-track group** treatment: foreground `LineName` plus a segmented brand bar, not a single line colour. Mixed-line rows lead with a line chip. The component does not infer merges from co-occurring line ids — Baker Street is the counterexample. This site’s Board opts in via a curated table; a library install passes `lineGroups` from the app.
