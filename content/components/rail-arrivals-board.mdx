...
Each bound shows `pageSize` trains, default 3. Hover the bound label and its rows to page further arrivals. A quiet `1/5` sits between the arrows. Touch screens keep the arrows visible. A short last page still occupies `pageSize` rows so paging does not jump the layout. A bound with only one page stays short.
...
`maxRows` is a per-bound cap after that order. It does not drop later lines. `pageSize` is how many trains each bound shows at once. Equal `timeToStation` keeps input order. The board does not mutate `data`.
