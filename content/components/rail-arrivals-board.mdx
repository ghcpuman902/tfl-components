...
Unattended keeps the bound height fixed and rotates later arrivals. Rank chips mark place in the full list. The first arrival stays pinned; later slots slide by one unless `pinAdvance` is `jump`.
...
| `pinAdvance` | `"slide" \| "jump"` | Unattended, when pinned: slide later slots by one (`1 2 3` then `1 3 4`), or jump a full window. Default `slide`. |
