import {
  RailSharedTrackGroupDemo,
  RailLineColumnsDemo,
  RailBoundColumnsDemo,
  RailLineAndBoundColumnsDemo,
  RailDestinationFilterDemo,
  RailDestinationFilterEmptyDemo,
  RailUnattendedDemo,
...
Each bound occupies `pageSize` arrival tiles, default 3 — including a short only page and a seeded empty group. Override per line with `pageSizeByLine` (ID-keyed map; falls back to `pageSize`). Extra pages sit in a native horizontal scroll-snap track. Each slide is locked to the track width, so destination wrap / abbreviation via [`StationName`](/docs/station-name-labels) is the same on page 1 and page 5 — a slide must not grow to the destination's max-content. On hover-capable pointers, hover the bound label and its rows for quiet arrows with a `1/5` count — clicks jump instantly. On touch screens, swipe the bound rows; half-opacity page dots stay visible in place of the arrows. New fetches update the current page in place. After inactivity on a later page, the bound returns to the first page. A short page with arrivals uses the last spare tile for **No more arrivals** and quiet dashes in any tiles between. Zero arrivals stay **No information**, then dashes — not an end-of-list state. Exact multiples of `pageSize` stay full. The pager hides when there is only one page.
...
Unattended keeps the bound height fixed and rotates later arrivals. Rank chips mark place in the full list. The first arrival stays pinned. Later slots chunk in windows of `pageSize - 1`; only a short final window overlaps (`1 2 3` then `1 4 5`, or `1 3 4` when four arrivals remain). Frames are not swipeable. New ordering is adopted at a frame boundary; countdown updates in place.
...
A train's destination is its terminus. Filter `data` before you pass it in. Prefer `destinationName` — `towards` is often blank on Elizabeth line and Overground. This does not prove the train calls at an intermediate station.

<RailDestinationFilterDemo />

```tsx
const toEpping = data.filter((row) =>
  (row.destinationName ?? "").includes("Epping")
)

<RailArrivalsBoard data={toEpping} stopName="Oxford Circus" />
```

When nothing matches, pass a destination-specific `emptyMessage`. Do not pass `lines` — that seeds empty line groups and hides the board-level message.

<RailDestinationFilterEmptyDemo />

```tsx
const toCockfosters = data.filter((row) =>
...
`maxRows` is a per-bound cap after that order. It does not drop later lines. `pageSize` is the fixed tile count per bound; `pageSizeByLine` overrides that per line id. Equal `timeToStation` keeps input order. The board does not mutate `data`.
...
| `pageSize` | `number` | Arrival tiles per bound, default 3. Short pages fill with dashes and **No more arrivals**. Hidden when only one page. |
...
| `behaviour` | `"interactive" \| "unattended"` | Interactive pager, or automatic frames with rank chips. |
| `pinFirst` | `boolean` | Unattended: keep the next service visible while later slots rotate. Default `true`. |
