...
`groupSortBy` only applies when `groupBy="route"`. `maxRows` is a per-route cap when grouped, and a list cap when flat. `pageSize` pages extra arrivals via scroll-snap. A short list with one page keeps its natural height. Once there is more than one page, the last page locks to `pageSize` tiles — dashes, then **No more arrivals**. Grouped: hover arrows on the route header. Flat: arrows stay visible in a trailing tile. Touch uses swipe plus half-opacity dots. The pager hides when there is only one page. Equal `timeToStation` keeps input order. The board does not mutate `data`.
