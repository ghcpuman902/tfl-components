...
6. **Every shared-track station, as separate families.** Circle / H&C / Met stay one family. Circle / District is a second family (southern loop, Victoria). District / H&C is a third (Aldgate East and the east end). Do not put District in the same exclusive-segment set as H&C / Met — Victoria would become shared and Circle loop trains would lose exclusive evidence. Baker Street merges Circle + H&C only. Paddington's Hammersmith-branch stop (`940GZZLUPAH`) stays Circle + H&C. Paddington Circle (`940GZZLUPAC`) — the station's own local platforms, the ones carrying the "Inner Rail" / "Outer Rail" qualifier — is a curated exception: `SHARED_TRACK_MERGE_INCLUDE` in `board-station-lines.ts` widens both the merge and the identity family to Circle + District + H&C there, because repeated live polling shows H&C vehicles (including ones dual-listed with District) landing on those same local platforms even though the static topology in `getSharedTrackSegments` doesn't mark H&C as shared with Circle/District at this stop.

   Don't confuse this with the actual Hammersmith terminus. `940GZZLUHSC` (the H&C/Circle building) is already correctly detected as Circle + H&C shared track by `getSharedTrackSegments` — **no curated override needed**. TfL tags every Circle-bound train through there as `hammersmith-city`; without the merge, Circle would seed as a permanently-empty section even though the platform is live. `940GZZLUHSD`, a separate physical building a short walk away for District + Piccadilly only, correctly gets none of this — the alias-leak guard in `lookupCuratedStopTable` blocks it from inheriting `940GZZLUHSC`'s merge. Don't merge the two buildings into one station: they don't share platforms.
